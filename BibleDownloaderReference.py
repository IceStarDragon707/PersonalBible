...
class Book:
	bookname = ''
	chapter = 0
	def __init__(self, bookname, chapter):
...
def getScriptContent(url, print_):
	...
	return [title, data]
def ExprotBibleBook(Book, font, text_size):
	return -1
def MakeAllBible():
	name = '-1'
	datapack = []
	...
	while num <= 1189:
		...
		if name == content[0].split(' ')[0]:
			...
			input('stop.')
	return -1
def list_FindBook(StrongSearch, FilterSearch, name):
	global list_
	collect = []
	if StrongSearch:
		for i in list_:
...
def runner_code__ch56(b_fastmode, name, len_, return_):
	...
	in_ = -1
	if name=='':
		name = input(tab+'Book Name ------>')
	if len_==-1:
		len_ = int(input(tab+'Chapter Number -------->'))
	while in_==-1:
		if FilterSearch:
			print(tab+'[FilterSearch Mode]')
			collect = list_FindBook(False, True, name)
			FilterSearch = False
		if len(collect)==0:
			print(tab+'[FullName Mode]')
			collect = list_FindBook(False, False, name)
		if len(collect)==0 and StrongSearch:
			print(tab+'[StrongSearch Mode]')
			collect = list_FindBook(StrongSearch, False, name)
			FilterSearch = False
		if len(collect)==0:
			print(tab+'[StrongSearch Mode]')
			collect = list_FindBook(True, False, name)
		if len(collect)>1:
			index = 1
			for c in collect:
				print(index, c.name())
			...
			in_ = -1
			collect = []
		name = input(tab+'Book Name, Again Please... ------>')
		in_ = -1
	...
	for l in list_:
		if l.name()==selec:
			...
			sum_ = sum_ + l.max()
	if len_>l.max():
		len_ = int(input(tab+'Chapter Number, Again Please... ----(<'+str(l.max())+')---->'))
...
def findBibleScript():
	...
	tab = tab+'\t'
	runner_code__ch56(ans, input(tab+'Book Name ------>'), int(input(tab+'Chapter Number -------->')), False)
	tab = tab[0:-1]
# if not 6:
def findBibleScript_CNC():
	## usable function -- Find Bible Script
	a, b = input('Book Name & Chapter Number ----(e.g.: 創世紀 5)---->').split(' ')
	tab = tab+'\t'
	runner_code__ch56(True, a, int(b), False)
	tab = tab[0:-1]
# if not 6:
def findBibleScript_CNCV():
	## usable function -- Find Bible Script Detail To Chapter
	a, b, c = input('Book Name & Chapter Number & ChapterNumber ----(e.g.: 啟示錄 5 8-9)---->').split(' ')
	tab = tab+'\t'
...
def kill():
	print('[Mode]\n\t\'m\' : show menu\t\t\'e\' : exit\n\t\'r\' : random bible script\n\t\'a\' : see all Book length\n\t\'f\' : find bible script by BookName And Book\n\t\'fs\' : fast search bible script by BookName And Book\n\t\'afs\' : advanced fast search bible script by BookName And Book And ChapterNumber\n\t\'cls\' : clear screen')
...
def int_RunTimer():
	m, s = str(datetime.today().strftime("%M:%S")).split(':')
